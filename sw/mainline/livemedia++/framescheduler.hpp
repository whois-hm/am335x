...
//		else if(_masterclock == AVMEDIA_TYPE_AUDIO)
//		{
//			return _audio_last_pts - (_pcmframe.first._index * std::get<0>(_pcmframe.first.audio_output_attr))/_audio_bps;
//		}
//		return 0.0;
//	}
//
//	virtual void usedframe( pixelframe_pts &rf)
//	{
//		double delay = 0.0;
//		double diff_pts  = 0.0;
//		double threshold = 0.0;
...
//		 */
//		_video[_video_type::last_presentation_time] = _video[_video_type::current_presentation_time];
//		_video[_video_type::current_presentation_time] = rf._pts;
//		_video[_video_type::global_timer] = (double)av_gettime();
//		delay = _video[_video_type::current_presentation_time] - _video[_video_type::last_presentation_time];
//
//		/*
...
//		rf(_videorational);
//	}
//	virtual void usedframe( pcmframe_pts &rf, int size_from_array = 0)
//	{
//		_audio_last_pts = rf.test_pts(_audiorational) /*input pts */ +
//				(size_from_array)/*input frame size */ /
//				(double)_audio_bps;
//	}
...
//	    _audiodiff_avgcoef = 0.0;
//		_audio_last_pts = 0.0;
//		_audio_bps = std::get<0>(_pcmframe.first.audio_output_attr) *
//				std::get<1>(_pcmframe.first.audio_output_attr) *
//				av_get_bytes_per_sample((enum AVSampleFormat)std::get<2>(_pcmframe.first.audio_output_attr));
//	}
//	void set_clock_master(enum AVMediaType masterclock)
//	{
//		_masterclock = masterclock;
...
//	}
//	avframescheduler(const avattr &attr,
//			enum AVMediaType masterclock,
//			const AVRational &videorational,
//			const AVRational &audiorational) :
//		framebuffering_type(attr),
//		_masterclock(masterclock),
//		_videorational(videorational),
//		_audiorational(audiorational)
//	{
//		_video[last_presentation_time] =
//		_video[current_presentation_time] =
//		_video[last_delay_time] =
...
//	pixel &operator >>(pixel &pf)
//	{
//		return framebuffering_type::operator >>(pf);
//	}
//	pcm_require &operator >>(pcm_require &pf)
//	{
//	    double diff = 0.0;
...
//	    int min_size = 0;
//	    int max_size = 0;
//		pcm_require &dump = framebuffering_type::operator >>(pf);
//		if(!dump.first.can_take())
//		{
//			return dump;
//		}
...
//		{
//			double audio_current_clock =
//					_audio_last_pts - (_pcmframe.first._index * std::get<0>(_pcmframe.first.audio_output_attr))/
//					_audio_bps;
//
//			diff = audio_current_clock - master_clock();
//			if(diff >= AV_NOSYNC_THRESHOLD)
...
//			int append_size = std::abs(manipulate_size);
//			int appended_len = dump.first.size() + append_size;
//			int bytesamplesize = av_get_bytes_per_sample((enum AVSampleFormat)std::get<2>(_pcmframe.first.audio_output_attr));
//			const uint8_t * lastsample = dump.first.read() + (dump.first.size() - bytesamplesize);
//
//
//			uint8_t new_pcm_buffer[appended_len];
...
//				(std::get<0>(_pcmframe.first.audio_output_attr),
//						std::get<1>(_pcmframe.first.audio_output_attr),
//						dump.first.size() / av_get_bytes_per_sample((enum AVSampleFormat)std::get<2>(_pcmframe.first.audio_output_attr)),
//				(enum AVSampleFormat)std::get<2>(_pcmframe.first.audio_output_attr)));
//
//		return dump;
//	}
